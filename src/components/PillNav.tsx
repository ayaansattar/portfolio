...
export default function PillNav({
  ...
  const setLogoRef: RefCallback<HTMLAnchorElement> = (el) => {
    logoRef.current = el;
  };

  const setCircleRef =
    (i: number): RefCallback<HTMLSpanElement> =>
    (el) => {
  ...
  const renderLogo = () => {
    ...
    const classNameLogo = "pill-logo";
    const img = <img src={logo} alt={logoAlt} ref={logoImgRef} />;
  ...
  const renderMobileLink = (item: PillNavItem, i: number) => {
    ...
    return (
      <a href={item.href} className={linkClass} onClick={closeMobileMenu}>
        {item.label}
      </a>
    );
  };
